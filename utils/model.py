...
class Network(nn.Module):
    ...
    def forward(self, x):
        ...
        Return#f:f
        output = self.detect.apply(self.numclasses, 0, 200, 0.01, 0.45f
            Depending on phase:
            test:
        ...
        if self.phase == "test":
            #print("self.priors is ", self.priors.type(type(x.data)))
            #output = self.detect(
            output = self.detect.apply(self.num_classes, 0, 200, 0.01, 0.45,
                                       loc.view(loc.size(0), -1, 4),                   # loc preds
                                       self.softmax(conf.view(conf.size(0), -1, self.num_classes)),                # conf preds
                                       self.priors.type(type(x.data)).to(torch.device("cuda:0"))                  # default boxes
            )
        else:
            output = (
...
def build_ssd(phase, size=300, num_classes=21):
    ...
    return Network(phase, size, base_, extras_, head_, num_classes)
